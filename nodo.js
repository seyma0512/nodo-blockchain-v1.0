...
app.post('/receive-data', upload.array('file'), async (req, res) => {
    ...
    const blockData = {
        ...
        node: 'nodo-1-blockchain-3001'
    };

    console.log('Enviando datos al Nodo 2 para validación...');
    try {
        // Enviar los datos al Nodo 2 para validación
        const response = await axios.post('https://nodo-2-blockchain-v1-0.onrender.com/validate-block', blockData);

        // Si el Nodo 2 valida el bloque, procedemos con la creación
        if (response.data.success) {
            console.log('Bloque validado por Nodo 2');
            ...
            return res.status(400).json({ 
                success: false, 
                message: 'El bloque no fue validado por Nodo 2' 
            });
        }
    } catch (error) {
        console.error('Error al enviar datos al Nodo 2:', error);
        return res.status(500).json({
            success: false,
            message: 'Error al validar el bloque con Nodo 2',
        });
    }
});
